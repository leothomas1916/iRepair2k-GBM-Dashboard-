...
export default function PremiumFeatures() {
  const features = [
    ...
    { label: "Google Map & Google Search", value: "Tracking", icon: MapIcon },
    ...
    {
      label: "Audit & Suspension Scores, Heat Maps",
      value: "Included",
      icon: ShieldCheck,
    },
    {
      label: "Bing & 20+ Publishers Integration",
      value: "Included",
      icon: Globe,
    },
    {
      label: "Social Media & Google Post Scheduling",
      value: "Included",
      icon: Calendar,
    },
    { label: "Dedicated Business Coach", value: "Included", icon: UserCheck },
    {
      label: "Competitor Sentiment Analysis",
      value: "Included",
      icon: SmilePlus,
    },
  ...
  return (
    <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-6 backdrop-blur-sm mt-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-[10px] uppercase tracking-widest text-orange-500 font-mono bg-orange-500/10 px-2 py-1 rounded-full">
          Pro Tier
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {features.map((feature, i) => {
          ...
          return (
            <div
              key={i}
              className="flex items-start gap-3 p-3 bg-zinc-950/50 rounded-xl border border-zinc-800/50 hover:border-orange-500/30 transition-colors"
            >
              <div className="p-2 bg-orange-500/10 rounded-lg shrink-0 mt-0.5">
                <Icon className="w-4 h-4 text-orange-400" />
              </div>
              <div>
                <div className="text-sm font-semibold text-zinc-200 leading-tight mb-0.5">
                  {feature.label}
                </div>
